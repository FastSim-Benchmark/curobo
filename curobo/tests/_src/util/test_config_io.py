...
@pytest.mark.parametrize(
    ...
    sorted(
        path
        for path in (Path(__file__).resolve().parents[3] / "content" / "configs").rglob("*")
        if path.suffix in (".yaml", ".yml", ".xrdf")
    ),
    ids=lambda path: str(path.relative_to(Path(__file__).resolve().parents[3] / "content")),
)
def test_bundled_configs_safe_roundtrip(config_path: Path, tmp_path: Path) -> None:
    """Preserve bundled robot, task and scene data through safe YAML serialization."""
    data = load_yaml(str(config_path))
    output = tmp_path / "config.yaml"
    write_yaml(data, str(output))
    assert load_yaml(str(output)) == data
